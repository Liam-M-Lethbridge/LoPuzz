...
fn create_queens_game(grid_size: u32) -> Vec<u32> {
    ...
    println!("current solutions: {}", count_solutions(&colour_grid, 0, grid_size, &mut temp_grid, 0));
    ...
    while ! unique {
        ...
        // let conflict = conflicts[rng().random_range(0..conflicts.len())];
            // record the conflict-colour pair
            conflict_pairs[conflict as usize].push(colour_grid[conflict as usize]);

            // change the colours of each clash
            print_grid(colour_grid.clone(), grid_size);
            println!("change to");
            change_colour(&mut colour_grid, conflict, grid_size, &queens_grid, &conflict_pairs[conflict as usize]);
            print_grid(colour_grid.clone(), grid_size);
            // observe any shape breaks caused by the prevous action and remove the colours
            ...
            println!("Ain't not way this shit unique");
        }
